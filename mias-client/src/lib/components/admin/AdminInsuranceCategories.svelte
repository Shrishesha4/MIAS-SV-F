<script lang="ts">
	...
	import { Briefcase, Building2, CircleOff, HeartPulse, Landmark, Loader2, PencilLine, Plus, Power, ShieldCheck, Trash2, Wallet } from 'lucide-svelte';
	...
	async function toggleCategoryActive(category: InsuranceCategory) {
		togglingId = category.id;
		try {
			const updated = await insuranceCategoriesApi.updateCategory(category.id, {
				is_active: !category.is_active,
			});
			categories = categories.map((item) => item.id === category.id ? { ...item, ...updated } : item);
			toastStore.addToast(`Insurance category ${updated.is_active ? 'enabled' : 'disabled'}`, 'success');
		} catch (error: any) {
			toastStore.addToast(error?.response?.data?.detail || 'Failed to update insurance category', 'error');
		} finally {
			togglingId = null;
		}
	}
							...
							<tr class="border-t border-slate-200 align-top">
								...
								<td class="px-4 py-4">
									<div class="flex flex-wrap gap-2">
										...
										<!-- Delete insurance category action hidden until admin disable flow replaces hard delete UI. -->
										<!--
