...
<div class="lg:h-screen lg:overflow-hidden lg:pb-0 pb-2">
	<div class="mx-auto flex max-w-md flex-col gap-3 px-4 pt-3 md:max-w-4xl md:px-6 lg:min-h-[86vh] lg:max-w-[1370px] lg:grid lg:grid-cols-[255px_minmax(0,1fr)] lg:gap-3 lg:px-5 lg:pt-4">
		{#if navItems.length > 0}
			<aside class="hidden lg:block lg:h-[86vh] lg:self-start">
				...
				>
					...
					<div class="admin-nav-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden bg-white/55">
					...
					<div class="shrink-0 px-4 py-4" style="background: linear-gradient(to bottom, rgba(255,255,255,0.78), rgba(246,248,252,0.92)); border-top: 1px solid rgba(190,200,214,0.42);">
						<button
		...
		<section class="min-w-0 space-y-4 lg:space-y-0">
			...
			{#if navItems.length > 0}
				<div
					class="admin-mobile-shell admin-mobile-tabs overflow-x-auto overflow-y-hidden rounded-[20px] border px-3 py-3 lg:hidden"
				...
				>
					<div class="flex min-w-max items-start justify-start gap-3 pr-1">
						{#each navItems as item (item.id)}
							{@const NavIcon = item.icon}
							{@const isActive = item.id === activeNav}
			...
			<div class="admin-content-shell sticky top-4 hidden h-[86vh] max-h-[86vh] overflow-hidden rounded-[18px] border lg:flex lg:flex-col" style="border-color: rgba(132,150,175,0.2); background: linear-gradient(to bottom, rgba(235,241,248,0.88), rgba(221,230,240,0.92)); box-shadow: 0 10px 24px rgba(95,113,136,0.12), inset 0 1px 0 rgba(255,255,255,0.84);">
				{#if children}
					<div class="admin-content-panel relative min-w-0 flex-1 overflow-y-auto overflow-x-hidden px-6 py-5">
						{@render children()}
					</div>
				{/if}
			</div>
			...
			{#if children}
				<div class="admin-content-panel relative min-w-0 overflow-x-hidden lg:hidden">
					{@render children()}
				</div>
			{/if}
		</section>
